...
-- Q11. Count how many films exist for each length where the film title
--      starts with 'T' and the count is more than 5.
...
--      lengths combined, ordering by rental rate first and length second.
...
-- Q14. Show all customers along with the total number of rentals
--      they have made, ordered from most to least rentals.
...
-- alternative version using LEFT JOIN + IS NULL :
-- SELECT f.film_id, f.title
-- FROM film f
